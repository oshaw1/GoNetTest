...
func (g *Generator) GenerateUploadAnalysisCharts(result *networkTesting.AverageSpeedTestResult) (*charts.Bar, error) {
	bar, err := generateUploadChart(result)
	if err != nil {
		return nil, err
	}
...
func (g *Generator) GenerateHistoricUploadAnalysisCharts(results []*networkTesting.AverageSpeedTestResult) (*charts.Bar, error) {
	if results == nil {
		return nil, fmt.Errorf("function called with no results")
	}

	barOverTime, err := generateUploadOverTimeBar(results)
	if err != nil {
		return nil, err
	}
...
func generateUploadChart(result *networkTesting.AverageSpeedTestResult) (*charts.Bar, error) {
	bar := charts.NewBar()
	...
	bar.SetXAxis(xAxis).
		AddSeries("Upload Speed", generateBarItems(speeds))

	return bar, nil
}

func generateUploadOverTimeBar(results []*networkTesting.AverageSpeedTestResult) (*charts.Bar, error) {
	bar := charts.NewBar()
	line := charts.NewLine()
	...
	bar.SetXAxis(xAxis).
		AddSeries("Upload Speed (Mbps)", generateBarItems(speeds)).
		SetSeriesOptions(
	...
	line.AddSeries("Upload Speed (Mbps)", generateLineItems(speeds)).
		SetSeriesOptions(
